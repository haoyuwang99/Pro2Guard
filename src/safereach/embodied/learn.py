...
def abstract_from_samples(dir): 
    ...
    M_BASE = f"safereach/embodied/dtmcs_{str(alpha)}"
    if not os.path.exists(M_BASE):
        os.mkdir(M_BASE)
    output_dir = M_BASE + dir[dir.rfind("/"):] + "/"
...
log_dir = [f for f in os.listdir('safereach/embodied/samples/') if f.startswith('log_raw_t')]
for d in log_dir:
    abstract_from_samples('safereach/embodied/samples/' + d) 
